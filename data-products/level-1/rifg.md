...
The RIFG product represents the ellipsoid and topography flattened wrapped interferogram generated from two L1 range-Doppler Single Look Complex ([RSLC](../level-1/rslc#product-overview)) products in the range-Doppler geometry of the earlier reference acquisition. The WGS84 ellipsoid is used as the reference surface for flat earth correction. 
...
The RIFG product includes the slant range and along-track sub-pixel offsets obtained from incoherent cross-correlation and used to generate the complex wrapped interferogram. If an offset product in range-Doppler coordinates ([ROFF](../level-1/roff#product-overview)) is available for the processed frame, the sub-pixel offset layers included in the RIFG product are obtained by optimally blending the multiresolution offset layers included in the ROFF product.
